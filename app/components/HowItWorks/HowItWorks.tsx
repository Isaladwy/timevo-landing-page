...
const HowItWorks = () => {
  return (
    <section className="w-full py-20 px-4 md:px-24 flex flex-col items-center">
      <h2 className="text-3xl md:text-4xl font-bold mb-12 text-center text-gray-100">
        How It Works
      </h2>
      <ol className="relative border-l-4 border-purple-300 max-w-2xl px-2 mx-auto">
        {steps.map((step, idx) => (
          <li key={idx} className="mb-12 ml-6 flex justify-center items-center last:mb-0 relative">
            <span className="absolute -left-6 top-8 flex items-center justify-center w-12 h-12 bg-purple-600 rounded-full text-white text-2xl font-bold shadow-lg">
              {step.icon}
            </span>
            <div className=" rounded-xl shadow-md p-6 ml-2">
              <h3 className="text-lg md:text-xl font-semibold mb-1 mt-1 text-gray-200">
                {step.title}
              </h3>
              <p className="text-gray-400 mb-1">{step.desc}</p>
            </div>
          </li>
        ))}
      </ol>
